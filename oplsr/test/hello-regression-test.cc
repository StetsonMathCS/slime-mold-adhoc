...
HelloRegressionTest::HelloRegressionTest () :
  TestCase ("Test OPLSR Hello messages generation"),
...
HelloRegressionTest::CreateNodes ()
{
  ...
  NodeContainer c;
  c.Create (2);
  // install TCP/IP & OPLSR
  OplsrHelper oplsr;
  InternetStackHelper internet;
  internet.SetRoutingHelper (oplsr);
  internet.Install (c);
  // Assign OPLSR RVs to specific streams
  int64_t streamsUsed = oplsr.AssignStreams (c, 0);
...
HelloRegressionTest::ReceivePktProbeA (Ptr<Socket> socket)
{
  ...
  const oplsr::MessageHeader::Hello &hello = msgHdr.GetHello ();
  ...
  std::vector<oplsr::MessageHeader::Hello::LinkMessage>::const_iterator iter;
