...
///
/// \brief Implementation of OPLSR agent and related classes.
///
/// This is the main file of this software because %OPLSR's behaviour is
/// implemented here.
///
...
#include "oplsr-routing-protocol.h"
...
///
/// We only use this value in order to define OPLSR_NEIGHB_HOLD_TIME.
///
#define OPLSR_REFRESH_INTERVAL   m_helloInterval


/********** Holding times **********/

/// Neighbor holding time.
#define OPLSR_NEIGHB_HOLD_TIME   Time (3 * OPLSR_REFRESH_INTERVAL)
/// Top holding time.
#define OPLSR_TOP_HOLD_TIME      Time (3 * m_tcInterval)
/// Dup holding time.
#define OPLSR_DUP_HOLD_TIME      Seconds (30)
/// MID holding time.
#define OPLSR_MID_HOLD_TIME      Time (3 * m_midInterval)
/// HNA holding time.
#define OPLSR_HNA_HOLD_TIME      Time (3 * m_hnaInterval)

/********** Link types **********/

/// Unspecified link type.
#define OPLSR_UNSPEC_LINK        0
/// Asymmetric link type.
#define OPLSR_ASYM_LINK          1
/// Symmetric link type.
#define OPLSR_SYM_LINK           2
/// Lost link type.
#define OPLSR_LOST_LINK          3

/********** Neighbor types **********/

/// Not neighbor type.
#define OPLSR_NOT_NEIGH          0
/// Symmetric neighbor type.
#define OPLSR_SYM_NEIGH          1
/// Asymmetric neighbor type.
#define OPLSR_MPR_NEIGH          2
...
#define OPLSR_MAXJITTER          (m_helloInterval.GetSeconds () / 4)
/// Maximum allowed sequence number.
#define OPLSR_MAX_SEQ_NUM        65535
/// Random number between [0-OPLSR_MAXJITTER] used to jitter OPLSR packet transmission.
#define JITTER (Seconds (m_uniformRandomVariable->GetValue (0, OPLSR_MAXJITTER)))


#define OPLSR_PORT_NUMBER 698
/// Maximum number of messages per packet.
#define OPLSR_MAX_MSGS           64

/// Maximum number of hellos per message (4 possible link types * 3 possible nb types).
#define OPLSR_MAX_HELLOS         12

/// Maximum number of addresses advertised on a message.
#define OPLSR_MAX_ADDRS          64


namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("OplsrRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::oplsr::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (OPLSR_WILL_DEFAULT),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (OPLSR_WILL_NEVER, "never",
                                    OPLSR_WILL_LOW, "low",
                                    OPLSR_WILL_DEFAULT, "default",
                                    OPLSR_WILL_HIGH, "high",
                                    OPLSR_WILL_ALWAYS, "always"))
    .AddTraceSource ("Rx", "Receive OPLSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::oplsr::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("Tx", "Send OPLSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_txPacketTrace),
                     "ns3::oplsr::RoutingProtocol::PacketTxRxTracedCallback")
    .AddTraceSource ("RoutingTableChanged", "The OPLSR routing table has changed.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_routingTableChanged),
                     "ns3::oplsr::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created oplsr::RoutingProtocol");
  ...
  m_packetSequenceNumber = OPLSR_MAX_SEQ_NUM;
  m_messageSequenceNumber = OPLSR_MAX_SEQ_NUM;
  m_ansn = OPLSR_MAX_SEQ_NUM;
...
void RoutingProtocol::DoInitialize ()
{
  ...
  NS_LOG_DEBUG ("Starting OPLSR on node " << m_mainAddress);

  Ipv4Address loopback ("127.0.0.1");

  bool canRunOplsr = false;
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      Ipv4Address addr = m_ipv4->GetAddress (i, 0).GetLocal ();
      if (addr == loopback)
      ...
      if (m_recvSocket == 0)
        {
          ...
          InetSocketAddress inetAddr (Ipv4Address::GetAny (), OPLSR_PORT_NUMBER);
          m_recvSocket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvOplsr,  this));
          if (m_recvSocket->Bind (inetAddr))
            {
              NS_FATAL_ERROR ("Failed to bind() OPLSR socket");
            }
      ...
      InetSocketAddress inetAddr (m_ipv4->GetAddress (i, 0).GetLocal (), OPLSR_PORT_NUMBER);
      socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvOplsr,  this));
  ...
  if (canRunOplsr)
    {
      ...
      NS_LOG_DEBUG ("OPLSR on node " << m_mainAddress << " started");
    }
}
...
//
// \brief Processes an incoming %OPLSR packet following \RFC{3626} specification.
void
RoutingProtocol::RecvOplsr (Ptr<Socket> socket)
{
  ...
  if (!receivedPacket->RemovePacketTag (interfaceInfo))
    {
      NS_ABORT_MSG ("No incoming interface on OPLSR message, aborting.");
    }
  ...
  NS_LOG_DEBUG ("OPLSR node " << m_mainAddress << " received a OPLSR packet from "
                             << senderIfaceAddr << " to " << receiverIfaceAddr);

  // All routing messages are sent from and to port RT_PORT,
  // so we check it.
  NS_ASSERT (inetSourceAddr.GetPort () == OPLSR_PORT_NUMBER);

  Ptr<Packet> packet = receivedPacket;

  oplsr::PacketHeader oplsrPacketHeader;
  packet->RemoveHeader (oplsrPacketHeader);
  NS_ASSERT (oplsrPacketHeader.GetPacketLength () >= oplsrPacketHeader.GetSerializedSize ());
  uint32_t sizeLeft = oplsrPacketHeader.GetPacketLength () - oplsrPacketHeader.GetSerializedSize ();
  ...
  while (sizeLeft)
    {
      ...
      NS_LOG_DEBUG ("Oplsr Msg received with type "
  ...
  m_rxPacketTrace (oplsrPacketHeader, messages);
...
//         }
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case oplsr::MessageHeader::HELLO_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OPLSR node " << m_mainAddress
            ...
            case oplsr::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OPLSR node " << m_mainAddress
            ...
            default:
              NS_LOG_DEBUG ("OPLSR message type " <<
        ...
        {
          NS_LOG_DEBUG ("OPLSR message is duplicated, not reading it.");
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType ()  != oplsr::MessageHeader::HELLO_MESSAGE)
            {
  ...
  // After processing all OPLSR messages, we must recompute the routing table
...
RoutingProtocol::MprComputation ()
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == OPLSR_WILL_NEVER)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == OPLSR_WILL_ALWAYS)
        {
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != OPLSR_WILL_NEVER)
            {
              nb2hopOk = true;
              break;
            }
...
void
RoutingProtocol::ProcessHello (const oplsr::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const oplsr::MessageHeader::Hello &hello = msg.GetHello ();
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Link Set for OPLSR Node " << m_mainAddress);
    for (LinkSet::const_iterator link = links.begin (); link != links.end (); link++)
      {
        NS_LOG_DEBUG (*link);
      }
    NS_LOG_DEBUG ("** END dump Link Set for OPLSR Node " << m_mainAddress);

    const NeighborSet &neighbors = m_state.GetNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump Neighbor Set for OPLSR Node " << m_mainAddress);
    for (NeighborSet::const_iterator neighbor = neighbors.begin (); neighbor != neighbors.end (); neighbor++)
      {
        NS_LOG_DEBUG (*neighbor);
      }
    NS_LOG_DEBUG ("** END dump Neighbor Set for OPLSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
#ifdef NS3_LOG_ENABLE
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TwoHopNeighbor Set for OPLSR Node " << m_mainAddress);
    for (TwoHopNeighborSet::const_iterator tuple = twoHopNeighbors.begin ();
         tuple != twoHopNeighbors.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TwoHopNeighbor Set for OPLSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
...
void
RoutingProtocol::ProcessTc (const oplsr::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const oplsr::MessageHeader::Tc &tc = msg.GetTc ();
...
#ifdef NS3_LOG_ENABLE
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                  << "s ** BEGIN dump TopologySet for OPLSR Node " << m_mainAddress);
    for (TopologySet::const_iterator tuple = topology.begin ();
         tuple != topology.end (); tuple++)
      {
        NS_LOG_DEBUG (*tuple);
      }
    NS_LOG_DEBUG ("** END dump TopologySet Set for OPLSR Node " << m_mainAddress);
  }
#endif // NS3_LOG_ENABLE
}

void
RoutingProtocol::ProcessMid (const oplsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const oplsr::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const oplsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const oplsr::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<oplsr::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ForwardDefault (oplsr::MessageHeader oplsrMessage,
                                 ...
                                 const Ipv4Address &senderAddress)
{
  ...
  if (duplicated != NULL && duplicated->retransmitted)
    {
      NS_LOG_LOGIC (Simulator::Now () << "Node " << m_mainAddress << " does not forward a message received"
                    " from " << oplsrMessage.GetOriginatorAddress () << " because it is duplicated");
      return;
    }
  ...
  if (oplsrMessage.GetTimeToLive () > 1)
    {
      const MprSelectorTuple *mprselTuple =
        m_state.FindMprSelectorTuple (GetMainAddress (senderAddress));
      if (mprselTuple != NULL)
        {
          oplsrMessage.SetTimeToLive (oplsrMessage.GetTimeToLive () - 1);
          oplsrMessage.SetHopCount (oplsrMessage.GetHopCount () + 1);
          // We have to introduce a random delay to avoid
          // synchronization with neighbors.
          QueueMessage (oplsrMessage, JITTER);
          retransmitted = true;
        }
    }
  ...
  if (duplicated != NULL)
    {
      duplicated->expirationTime = now + OPLSR_DUP_HOLD_TIME;
  ...
  // ...or create a new one
  else
    {
      ...
      newDup.address = oplsrMessage.GetOriginatorAddress ();
      newDup.sequenceNumber = oplsrMessage.GetMessageSequenceNumber ();
      newDup.expirationTime = now + OPLSR_DUP_HOLD_TIME;
      ...
      Simulator::Schedule (OPLSR_DUP_HOLD_TIME,
...
void
RoutingProtocol::QueueMessage (const oplsr::MessageHeader &message, Time delay)
{
...
{
  NS_LOG_DEBUG ("OPLSR node " << m_mainAddress << " sending a OPLSR packet");

  m_packets_sent++;

  // Add a header
  oplsr::PacketHeader header;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_sendSockets.begin (); i != m_sendSockets.end (); i++)
    {
      ...
      i->first->SendTo (pkt, 0, InetSocketAddress (bcast, OPLSR_PORT_NUMBER));
    }
}
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  NS_LOG_DEBUG ("Oplsr node " << m_mainAddress << ": SendQueuedMessages");

  MessageList msglist;

  for (std::vector<oplsr::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  oplsr::MessageHeader msg;
  Time now = Simulator::Now ();

  msg.SetVTime (OPLSR_NEIGHB_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (1);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  oplsr::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<oplsr::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      oplsr::MessageHeader::Hello::LinkMessage linkMessage;
  ...
  NS_LOG_DEBUG ("OPLSR HELLO message size: " << int (msg.GetSerializedSize ())
...
RoutingProtocol::SendTc ()
{
  ...
  msg.SetVTime (OPLSR_TOP_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());

  oplsr::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
RoutingProtocol::SendMid ()
{
  ...
  msg.SetVTime (OPLSR_MID_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetVTime (OPLSR_HNA_HOLD_TIME);
  msg.SetOriginatorAddress (m_mainAddress);
  msg.SetTimeToLive (255);
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  oplsr::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<oplsr::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      oplsr::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
RoutingProtocol::SetRoutingTableAssociation (Ptr<Ipv4StaticRouting> routingTable)
{
      ...
      for (uint32_t i = 0; i < m_routingTableAssociation->GetNRoutes (); i++)
        {
          ...
          // If the outgoing interface for this route is a non-oplsr interface
          if (UsesNonOplsrOutgoingInterface (route))
            {
  ...
  // add the routes using non-oplsr outgoing interfaces to the list
...
bool
RoutingProtocol::UsesNonOplsrOutgoingInterface (const Ipv4RoutingTableEntry &route)
{
  std::set<uint32_t>::const_iterator ci = m_interfaceExclusions.find (route.GetInterface ());
  // The outgoing interface is a non-OPLSR interface if a match is found
  // before reaching the end of the list of excluded interfaces
  return ci != m_interfaceExclusions.end ();
}

void
RoutingProtocol::LinkSensing (const oplsr::MessageHeader &msg,
                              const oplsr::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  bool updated = false;
  bool created = false;
  NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Oplsr node " << m_mainAddress
  ...
  for (std::vector<oplsr::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
       ...
       linkMessage++)
    {
      ...
      switch (nt)
        {
        case OPLSR_NOT_NEIGH:
          neighborTypeName = "NOT_NEIGH";
          break;
        case OPLSR_SYM_NEIGH:
          neighborTypeName = "SYM_NEIGH";
          break;
        case OPLSR_MPR_NEIGH:
      ...
      if ((lt == OPLSR_SYM_LINK && nt == OPLSR_NOT_NEIGH)
          || (nt != OPLSR_SYM_NEIGH && nt != OPLSR_MPR_NEIGH
              && nt != OPLSR_NOT_NEIGH))
        {
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == OPLSR_LOST_LINK)
              ...
              else if (lt == OPLSR_SYM_LINK || lt == OPLSR_ASYM_LINK)
                {
                  ...
                  link_tuple->time = link_tuple->symTime + OPLSR_NEIGHB_HOLD_TIME;
                  updated = true;
                }
              else
                {
...
void
RoutingProtocol::PopulateNeighborSet (const oplsr::MessageHeader &msg,
                                      const oplsr::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const oplsr::MessageHeader &msg,
                                            const oplsr::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();

  NS_LOG_DEBUG ("Oplsr node " << m_mainAddress << ": PopulateTwoHopNeighborSet BEGIN");
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<oplsr::MessageHeader::Hello::LinkMessage> LinkMessageVec;
           ...
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          for (std::vector<Ipv4Address>::const_iterator nb2hop_addr_iter =
               ...
               nb2hop_addr_iter++)
            {
              ...
              if (neighborType == OPLSR_SYM_NEIGH || neighborType == OPLSR_MPR_NEIGH)
                {
              ...
              else if (neighborType == OPLSR_NOT_NEIGH)
                {
  ...
  NS_LOG_DEBUG ("Oplsr node " << m_mainAddress << ": PopulateTwoHopNeighborSet END");
}

void
RoutingProtocol::PopulateMprSelectorSet (const oplsr::MessageHeader &msg,
                                         const oplsr::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
...
OPLSR::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         OPLSR::node_id (ra_addr ()),
         OPLSR::node_id (ch->next_hop ()));
  ...
  OPLSR_link_tuple* link_tuple = state_.find_link_tuple (ch->next_hop ());
  if (link_tuple != NULL)
    {
      link_tuple->lost_time () = now + OPLSR_NEIGHB_HOLD_TIME;
      link_tuple->time ()      = now + OPLSR_NEIGHB_HOLD_TIME;
      nb_loss (link_tuple);
    }
  drop (p, DROP_RTR_MAC_CALLBACK);
}
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, OPLSR_WILL_DEFAULT);
...
//         Simulator::Now (),
//         OPLSR::node_id(ra_addr()),
//         OPLSR::node_id(tuple->neighborMainAddr),
//         ((tuple->status() == OPLSR_STATUS_SYM) ? "sym" : "not_sym"));
...
//         OPLSR::node_id(ra_addr()),
//         OPLSR::node_id(tuple->neighborMainAddr),
//         OPLSR::node_id(tuple->twoHopNeighborAddr));
...
{
  m_ansn = (m_ansn + 1) % (OPLSR_MAX_SEQ_NUM + 1);
}
...
//         Simulator::Now (),
//         OPLSR::node_id(ra_addr()),
//         OPLSR::node_id(tuple->dest_addr()),
//         OPLSR::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         OPLSR::node_id(ra_addr()),
//         OPLSR::node_id(tuple->main_addr()),
//         OPLSR::node_id(tuple->iface_addr()));
...
{
  m_packetSequenceNumber = (m_packetSequenceNumber + 1) % (OPLSR_MAX_SEQ_NUM + 1);
  return m_packetSequenceNumber;
}

uint16_t RoutingProtocol::GetMessageSequenceNumber ()
{
  m_messageSequenceNumber = (m_messageSequenceNumber + 1) % (OPLSR_MAX_SEQ_NUM + 1);
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      if (oif && m_ipv4->GetInterfaceForDevice (oif) != static_cast<int> (interfaceIdx))
        {
          ...
          NS_LOG_DEBUG ("Oplsr node " << m_mainAddress
      ...
      if (numOifAddresses == 1)
        ...
        {
          /// \todo Implement IP aliasing and OPLSR
          NS_FATAL_ERROR ("XXX Not implemented yet:  IP aliasing and OPLSR");
        }
