...
#define OPLSR_MSG_HEADER_SIZE 12
#define OPLSR_PKT_HEADER_SIZE 4

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("OplsrHeader");

namespace oplsr {


/// Scaling factor used in RFC 3626.
#define OPLSR_C 0.0625
...
SecondsToEmf (double seconds)
{
  ...
  NS_ASSERT_MSG (seconds >= OPLSR_C, "SecondsToEmf - Can not convert a value less than OPLSR_C");

  // find the largest integer 'b' such that: T/C >= 2^b
  for (b = 1; (seconds / OPLSR_C) >= (1 << b); ++b)
    {
    }
  NS_ASSERT ((seconds / OPLSR_C) < (1 << b));
  b--;
  NS_ASSERT ((seconds / OPLSR_C) >= (1 << b));

  // compute the expression 16*(T/(C*(2^b))-1), which may not be a integer
  double tmp = 16 * (seconds / (OPLSR_C * (1 << b)) - 1);
...
///
/// \param oplsr_format number of seconds in mantissa/exponent format.
/// \return the decimal number of seconds.
///
double
EmfToSeconds (uint8_t oplsrFormat)
{
  int a = (oplsrFormat >> 4);
  int b = (oplsrFormat & 0xf);
  // value = C*(1+a/16)*2^b [in seconds]
  return OPLSR_C * (1 + a / 16.0) * (1 << b);
}
...
{
  static TypeId tid = TypeId ("ns3::oplsr::PacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("Oplsr")
...
{
  static TypeId tid = TypeId ("ns3::oplsr::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("Oplsr")
...
{
  uint32_t size = OPLSR_MSG_HEADER_SIZE;
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  size = OPLSR_MSG_HEADER_SIZE;
  switch (m_messageType)
    {
    case MID_MESSAGE:
      size += m_message.mid.Deserialize (i, m_messageSize - OPLSR_MSG_HEADER_SIZE);
      break;
    case HELLO_MESSAGE:
      size += m_message.hello.Deserialize (i, m_messageSize - OPLSR_MSG_HEADER_SIZE);
      break;
    case TC_MESSAGE:
      size += m_message.tc.Deserialize (i, m_messageSize - OPLSR_MSG_HEADER_SIZE);
      break;
    case HNA_MESSAGE:
      size += m_message.hna.Deserialize (i, m_messageSize - OPLSR_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
