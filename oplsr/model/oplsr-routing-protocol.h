...
#ifndef OPLSR_AGENT_IMPL_H
#define OPLSR_AGENT_IMPL_H

#include "oplsr-header.h"
#include "ns3/test.h"
#include "oplsr-state.h"
#include "oplsr-repositories.h"
...
class OplsrMprTestCase;

namespace ns3 {
namespace oplsr {

///
/// \defgroup oplsr OPLSR Routing
/// This section documents the API of the ns-3 OPLSR module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup oplsr
/// An %OPLSR's routing table entry.
...
/// \ingroup oplsr
///
/// \brief OPLSR routing protocol for IPv4
///
...
public:
   ...
   * \return the list of routing table entries discovered by OPLSR
   */
...
public:
   ...
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "OplsrState::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
   ...
   *         to the OPLSR routing protocol. Entries from this associated
   *         routing table that use non-oplsr outgoing interfaces are added
...
private:
  ...
  OplsrState m_state;  //!< Internal state with all needed data structs.
  ...
  /**
   * Receive an OPLSR message.
   * \param socket The receiving socket.
   */
  void RecvOplsr (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-OPLSR outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-OPLSR interface, false otherwise.
   */
  bool UsesNonOplsrOutgoingInterface (const Ipv4RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the OPLSR agent.
   *
  ...
  oplsr::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief OPLSR's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param oplsrMessage The %OPLSR message which must be forwarded.
   ...
   */
  void ForwardDefault (oplsr::MessageHeader oplsrMessage,
                       DuplicateTuple *duplicated,
                       const Ipv4Address &localIface,
                       const Ipv4Address &senderAddress);

  /**
   * \brief Enques an %OPLSR message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %OPLSR messages in
   * a same %OPLSR packet.
   *
   * \param message the %OPLSR message which must be sent.
   * \param delay maximum delay the %OPLSR message is going to be buffered.
   */
  void QueueMessage (const oplsr::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %OPLSR packets as needed in order to send all buffered
   * %OPLSR messages.
   *
   * Maximum number of messages which can be contained in an %OPLSR packet is
   * dictated by OPLSR_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %OPLSR HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %OPLSR TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %OPLSR MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %OPLSR HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %OPLSR message which contains the HELLO message.
   ...
   */
  void ProcessHello (const oplsr::MessageHeader &msg,
   ...
   *
   * \param msg The %OPLSR message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const oplsr::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %OPLSR message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const oplsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   * \param msg the %OPLSR message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const oplsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const oplsr::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const oplsr::MessageHeader &msg,
                            const oplsr::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const oplsr::MessageHeader &msg,
                                  const oplsr::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const oplsr::MessageHeader &msg,
                               const oplsr::MessageHeader::Hello &hello);
  ...
  // (reason: for OPLSR Link Sensing we need to know on which interface
