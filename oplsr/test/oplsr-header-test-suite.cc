 ...
 * \ingroup tests
 *
 * Check Emf oplsr time conversion
 */
class OplsrEmfTestCase : public TestCase
{
public:
  OplsrEmfTestCase ();
...
OplsrEmfTestCase::OplsrEmfTestCase ()
  : TestCase ("Check Emf oplsr time conversion")
{
}
void
OplsrEmfTestCase::DoRun (void)
{
  for (int time = 1; time <= 30; time++)
    {
      uint8_t emf = oplsr::SecondsToEmf (time);
      double seconds = oplsr::EmfToSeconds (emf);
 ...
 * \ingroup tests
 *
 * Check Mid oplsr messages
 */
class OplsrMidTestCase : public TestCase
{
public:
  OplsrMidTestCase ();
  virtual void DoRun (void);
...
OplsrMidTestCase::OplsrMidTestCase ()
  : TestCase ("Check Mid oplsr messages")
{
}
void
OplsrMidTestCase::DoRun (void)
{
  Packet packet;

  {
    oplsr::PacketHeader hdr;
    oplsr::MessageHeader msg1;
    oplsr::MessageHeader::Mid &mid1 = msg1.GetMid ();
    oplsr::MessageHeader msg2;
    oplsr::MessageHeader::Mid &mid2 = msg2.GetMid ();
    ...
    msg2.SetMessageType (oplsr::MessageHeader::MID_MESSAGE);
    msg2.SetMessageSequenceNumber (7);

    // Build an OPLSR packet header
...

  {
    ...
    uint32_t sizeLeft = hdr.GetPacketLength () - hdr.GetSerializedSize ();
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg1.GetMessageType (), oplsr::MessageHeader::MID_MESSAGE, "204");
    ...
    }
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (msg2.GetMessageType (), oplsr::MessageHeader::MID_MESSAGE, "212");
 ...
 * \ingroup tests
 *
 * Check Hello oplsr messages
 */
class OplsrHelloTestCase : public TestCase
{
public:
  OplsrHelloTestCase ();
  virtual void DoRun (void);
...
OplsrHelloTestCase::OplsrHelloTestCase ()
  : TestCase ("Check Hello oplsr messages")
{
}
void
OplsrHelloTestCase::DoRun (void)
{
  Packet packet;
  oplsr::MessageHeader msgIn;
  oplsr::MessageHeader::Hello &helloIn = msgIn.GetHello ();

  helloIn.SetHTime (Seconds (7));
  helloIn.willingness = 66;

  {
    oplsr::MessageHeader::Hello::LinkMessage lm1;
    lm1.linkCode = 2;
    ...
    oplsr::MessageHeader::Hello::LinkMessage lm2;
    lm2.linkCode = 3;
  ...
  oplsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  oplsr::MessageHeader::Hello &helloOut = msgOut.GetHello ();
 ...
 * \ingroup tests
 *
 * Check Tc oplsr messages
 */
class OplsrTcTestCase : public TestCase
{
public:
  OplsrTcTestCase ();
  virtual void DoRun (void);
...
OplsrTcTestCase::OplsrTcTestCase ()
  : TestCase ("Check Tc oplsr messages")
{
}
void
OplsrTcTestCase::DoRun (void)
{
  Packet packet;
  oplsr::MessageHeader msgIn;
  oplsr::MessageHeader::Tc &tcIn = msgIn.GetTc ();
  ...
  oplsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  oplsr::MessageHeader::Tc &tcOut = msgOut.GetTc ();
 ...
 * \ingroup tests
 *
 * Check Hna oplsr messages
 */
class OplsrHnaTestCase : public TestCase
{
public:
  OplsrHnaTestCase ();
  virtual void DoRun (void);
...
OplsrHnaTestCase::OplsrHnaTestCase ()
  : TestCase ("Check Hna oplsr messages")
{
}

void
OplsrHnaTestCase::DoRun (void)
{
  Packet packet;
  oplsr::MessageHeader msgIn;
  oplsr::MessageHeader::Hna &hnaIn = msgIn.GetHna ();

  hnaIn.associations.push_back ((oplsr::MessageHeader::Hna::Association)
  ...
  oplsr::MessageHeader msgOut;
  packet.RemoveHeader (msgOut);
  oplsr::MessageHeader::Hna &hnaOut = msgOut.GetHna ();
 ...
 * \ingroup tests
 *
 * Check oplsr header messages
 */
class OplsrTestSuite : public TestSuite
{
public:
  OplsrTestSuite ();
};

OplsrTestSuite::OplsrTestSuite ()
  : TestSuite ("routing-oplsr-header", UNIT)
{
  AddTestCase (new OplsrHnaTestCase (), TestCase::QUICK);
  AddTestCase (new OplsrTcTestCase (), TestCase::QUICK);
  AddTestCase (new OplsrHelloTestCase (), TestCase::QUICK);
  AddTestCase (new OplsrMidTestCase (), TestCase::QUICK);
  AddTestCase (new OplsrEmfTestCase (), TestCase::QUICK);
}

static OplsrTestSuite g_oplsrTestSuite; //!< Static variable for test initialization
