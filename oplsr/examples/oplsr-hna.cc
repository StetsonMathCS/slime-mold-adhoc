...
//
// One way is to use oplsr::RoutingProtocol::SetRoutingTableAssociation ()
// to use which you may run:
//
// ./waf --run "oplsr-hna --assocMethod1=1"
//
// The other way is to use oplsr::RoutingProtocol::AddHostNetworkAssociation ()
// to use which you may run:
//
// ./waf --run "oplsr-hna --assocMethod2=1"
//
...
NS_LOG_COMPONENT_DEFINE ("OplsrHna");
...
int main (int argc, char *argv[])
{
  ...
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, oplsrNodes);

  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", DataRateValue (DataRate (5000000)));
  csma.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (2)));
  NetDeviceContainer csmaDevices = csma.Install (NodeContainer (csmaNodes.Get (0), oplsrNodes.Get (1)));
  ...
  mobility.Install (oplsrNodes);

  OplsrHelper oplsr;

  // Specify Node B's csma device as a non-OPLSR device.
  oplsr.ExcludeInterface (oplsrNodes.Get (1), 2);
  ...
  InternetStackHelper internet_oplsr;
  internet_oplsr.SetRoutingHelper (list); // has effect on the next Install ()
  internet_oplsr.Install (oplsrNodes);
  ...
  Ptr<Socket> source = Socket::CreateSocket (oplsrNodes.Get (0), tid);
  InetSocketAddress remote = InetSocketAddress (Ipv4Address ("172.16.1.1"), 80);
  source->Connect (remote);

  // Obtain oplsr::RoutingProtocol instance of gateway node
  // (namely, node B) and add the required association
  Ptr<Ipv4> stack = oplsrNodes.Get (1)->GetObject<Ipv4> ();
  Ptr<Ipv4RoutingProtocol> rp_Gw = (stack->GetRoutingProtocol ());
  Ptr<Ipv4ListRouting> lrp_Gw = DynamicCast<Ipv4ListRouting> (rp_Gw);

  Ptr<oplsr::RoutingProtocol> oplsrrp_Gw;

  for (uint32_t i = 0; i < lrp_Gw->GetNRoutingProtocols ();  i++)
    {
      int16_t priority;
      Ptr<Ipv4RoutingProtocol> temp = lrp_Gw->GetRoutingProtocol (i, priority);
      if (DynamicCast<oplsr::RoutingProtocol> (temp))
        {
          oplsrrp_Gw = DynamicCast<oplsr::RoutingProtocol> (temp);
        }
    }
  ...
  if (assocMethod1)
    {
      ...
      // which are associated with non-OPLSR interfaces specified above.
      hnaEntries->AddNetworkRouteTo (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"), uint32_t (2), uint32_t (1));
      oplsrrp_Gw->SetRoutingTableAssociation (hnaEntries);
    }

  if (assocMethod2)
    {
      // Specify the required associations directly.
      oplsrrp_Gw->AddHostNetworkAssociation (Ipv4Address ("172.16.1.0"), Ipv4Mask ("255.255.255.0"));
    }

  // Tracing
  wifiPhy.EnablePcap ("oplsr-hna", devices);
  csma.EnablePcap ("oplsr-hna", csmaDevices, false);
